...
# ─── Assenze tecnici (vista Triage + pagina settimanale) ────────────────────
# Le assenze vivono nella tabella condivisa `attendance.Absence` (mezza
# giornata MAT/POM, vedi attendance/models.py) e sono già usate in sola
# lettura da Triage/Statistiche più sotto in questo file.
#
# Questo endpoint espone invece un CRUD dedicato per i tecnici ServiceNow,
# volutamente SEPARATO da `attendance.api.AbsenceViewSet`: quel viewset
# applica le regole del roster Piano Ferie (che esclude i tecnici Philips e
# limita i non-coordinatori alle proprie proposte di ferie), regole non
# adatte a un dispatcher ServiceNow che deve poter segnare assente QUALSIASI
# tecnico (Philips o Biotron) per QUALSIASI motivo. Qui l'autorizzazione è
# invece quella già in uso nel pannello Triage: permesso
# `servicenow.view_servicenowcase` in lettura, `servicenow.change_servicenowcase`
# in scrittura. Le voci create da qui nascono già VALIDATA (inserimento
# amministrativo diretto, non proposta del dipendente), come fa il
# coordinatore nel Piano Ferie.

class SnTechnicianAbsenceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "reason", "reason_label", "note", "time_from", "time_to", "is_hourly",
            "status", "created_at", "updated_at",
        ]
        read_only_fields = [
            "id", "user_name", "reason_label", "day_part_label", "is_hourly",
            "status", "created_at", "updated_at",
        ]
    ...
    def validate(self, attrs):
        ...
        # righe attive: senza questo controllo, un duplicato arriva fino al DB
        # e Postgres solleva un IntegrityError non gestito → 500 (bug segnalato
        # dopo la release archie-sn-absences-01). DRF non genera in automatico
        # un validator per i vincoli condizionati (UniqueConstraint con
        # `condition=`), quindi va fatto a mano qui.
        date_val = attrs.get("date", getattr(self.instance, "date", None))
        day_part_val = attrs.get("day_part", getattr(self.instance, "day_part", None))
        if user is not None and date_val and day_part_val:
            conflict = Absence.objects.filter(
                user=user, date=date_val, day_part=day_part_val, deleted_at__isnull=True,
            )
            if self.instance is not None:
                conflict = conflict.exclude(pk=self.instance.pk)
            if conflict.exists():
                day_part_label = dict(DayPart.choices).get(day_part_val, day_part_val)
                raise serializers.ValidationError(
                    f"Esiste già una voce per il {date_val} ({day_part_label}). "
                    "Modificala o eliminala prima di aggiungerne una nuova."
                )
        return attrs


class SnAbsencePermission(BasePermission):
    """Lettura: chi vede il Triage (servicenow.view_servicenowcase). Scrittura:
    chi può modificare i case (servicenow.change_servicenowcase), stesso
    permesso già usato dal frontend per mostrare i controlli di gestione."""
...
class SnTechnicianAbsenceViewSet(SoftDeleteAuditMixin, RestoreActionMixin, viewsets.ModelViewSet):
    """CRUD assenze per i tecnici ServiceNow (Philips + Biotron), a mezza
    ...
    filterset_fields   = ["user", "day_part", "reason"]

    def get_queryset(self):
        qs = (
            Absence.objects
            .filter(deleted_at__isnull=True, user__profile__is_servicenow_technician=True)
    ...
    def perform_create(self, serializer):
        instance = serializer.save(
            status=AbsenceStatus.VALIDATA,
            validated_by=self.request.user, validated_at=timezone.now(),
        ...
        data = [
            {
                ...
                "category": "philips" if bool(getattr(u.profile, "is_philips", False)) else "biotron",
            }
            for u in users
        ]
        return Response(data)

    @action(detail=False, methods=["post"], url_path="full-day")
    def full_day(self, request):
        """Crea in un'unica transazione le due voci mattina+pomeriggio per un
        giorno intero. Volutamente NON delega a due POST separate lato
        frontend: se una delle due fasce fosse già occupata, due chiamate
        indipendenti potrebbero salvarne una e fallire sull'altra (scrittura
        parziale), oltre a rischiare l'IntegrityError/500 visto in produzione.
        Qui si valida prima ENTRAMBE le fasce e si scrive tutto o niente.
        """
        user_id = request.data.get("user")
        date_val = request.data.get("date")
        reason = request.data.get("reason", AbsenceReason.FERIE)
        note = request.data.get("note", "")

        if not user_id or not date_val:
            return Response({"detail": "user e date sono obbligatori."}, status=status.HTTP_400_BAD_REQUEST)

        user_obj = get_object_or_404(User, pk=user_id)
        try:
            is_sn_tech = bool(user_obj.profile.is_servicenow_technician)
        except Exception:
            is_sn_tech = False
        if not is_sn_tech:
            return Response({"user": "L'utente selezionato non è un tecnico ServiceNow."}, status=status.HTTP_400_BAD_REQUEST)

        existing_parts = list(
            Absence.objects
            .filter(user=user_obj, date=date_val, deleted_at__isnull=True)
            .values_list("day_part", flat=True)
        )
        if existing_parts:
            labels = [dict(DayPart.choices).get(p, p) for p in existing_parts]
            return Response(
                {"detail": f"Esistono già voci per il {date_val} ({', '.join(labels)}). Modificale o eliminale prima."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        created = []
        with transaction.atomic():
            for part in (DayPart.MATTINA, DayPart.POMERIGGIO):
                instance = Absence.objects.create(
                    user=user_obj, date=date_val, day_part=part, reason=reason, note=note,
                    status=AbsenceStatus.VALIDATA,
                ...
                log_event(
                    ...
                    changes={
                        "date": {"from": None, "to": to_change_value_for_field("date", date_val)},
                        "day_part": {"from": None, "to": to_change_value_for_field("day_part", part)},
                        "reason": {"from": None, "to": to_change_value_for_field("reason", reason)},
                    },
                    request=request,
                )
