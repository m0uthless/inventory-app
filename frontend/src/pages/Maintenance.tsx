...
function FieldRow({
  ...
  return (
    <Stack direction="row" spacing={1} alignItems="center" sx={{ py: 0.75 }}>
      ...
      {v ? (
        <Tooltip title="Copia">
          <IconButton aria-label="Copia" size="small" onClick={() => copyText(v)}>
...
async function fetchComputedDueDate(
  ...
  try {
    ...
    const res = await api.get<{ next_due_date: string }>('/maintenance-plans/compute-due-date/', {
      params,
    })
...
function PlansTab() {
  ...
  const { rows: customers } = useDrfList<CustomerItem>('/customers/', {
  ...
  React.useEffect(() => {
    api
      .get('/inventory-types/', { params: { ordering: 'label' } })
  ...
  const save = async () => {
    ...
    if (!form.inventory_types.length) {
      toast.warning('Seleziona almeno un tipo di inventario.')
      return
    }
    if (!form.title.trim()) {
      toast.warning('Inserisci un titolo.')
      return
    }
    if (!form.next_due_date) {
      toast.warning('La data prevista è obbligatoria.')
      return
    }
    if (form.schedule_type === 'interval' && (!form.interval_value || !form.interval_unit)) {
      toast.warning("Specifica valore e unità dell'intervallo.")
      return
    }
    if (form.schedule_type === 'fixed_date' && (!form.fixed_month || !form.fixed_day)) {
      toast.warning('Specifica giorno e mese per la scadenza fissa.')
      return
    }
    ...
    const payload: any = {
      ...
      interval_value:
        form.schedule_type === 'interval' ? Number(form.interval_value) || null : null,
  ...
  const doRestore = async () => {
    ...
    try {
      ...
      toast.success('Piano ripristinato ✅')
      reload()
      const r = await api.get<PlanRow>(`/maintenance-plans/${detail.id}/`)
      setDetail(r.data)
    } catch (e) {
      toast.error(apiErrorToMessage(e))
    } finally {
  ...
  const columns: GridColDef<PlanRow>[] = React.useMemo(
    () => [
      ...
      {
        field: 'inventory_type_labels',
        headerName: 'Tipi inventario',
        flex: 1,
        minWidth: 200,
  ...
  const selectedTypeLabels = invTypes
    .filter((t) => form.inventory_types.includes(t.id))
    .map((t) => t.label)
    ...
    <>
      <EntityListCard
        toolbar={{
          ...
          onReset: () => {
            grid.reset(['p_customer', 'p_due', 'p_active'])
            setCustomerId('')
            setDue('')
            setAct('')
          },
        ...
        grid={{
          ...
          onRowSelectionModelChange: (m) => setSel(m as GridRowSelectionModel),
          sx: GRID_SX,
        }}
      >
        <FilterChip
          ...
          <DetailDrawerHeader
            ...
            subtitle={
              detail?.customer_code
                ? `${detail.customer_code} — ${detail.customer_name}`
                : undefined
            }
            onClose={() => grid.setOpenId(null)}
            divider={false}
                ...
                <Can perm={PERMS.maintenance.plan.change}>
                  {detail?.deleted_at ? (
                    <Tooltip title="Ripristina">
                      <span>
                        <IconButton aria-label="Ripristina" onClick={doRestore} disabled={resBusy}>
                    ...
                    <Tooltip title="Modifica">
                      <span>
                        <IconButton aria-label="Modifica" onClick={openEdit} disabled={!detail}>
                          <EditIcon />
                        </IconButton>
                      </span>
                    </Tooltip>
          ...
          {dlLoading ? (
            ...
            <>
              <Stack direction="row" spacing={0.75} flexWrap="wrap">
                <Chip
                  size="small"
                  label={detail.is_active ? 'Attivo' : 'Non attivo'}
                  color={detail.is_active ? 'success' : 'default'}
                />
                {detail.deleted_at && <Chip size="small" label="Eliminato" color="error" />}
              </Stack>

              <Typography variant="subtitle2" sx={{ opacity: 0.6, mt: 0.5 }}>
                Copertura
              </Typography>
              <FieldRow
                label="Tipi inventario"
                value={(detail.inventory_type_labels ?? []).join(', ')}
              />
              <FieldRow
                label="Inventari attivi"
                value={detail.covered_count != null ? String(detail.covered_count) : '—'}
              />
              ...
              <FieldRow
                label="Tipo"
                value={detail.schedule_type === 'interval' ? 'Intervallo' : 'Data fissa'}
              />
          ...
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            <FormControl size="small" fullWidth required>
              ...
              <Select
                ...
                onChange={(e) =>
                  ff({ customer: e.target.value === '' ? '' : Number(e.target.value) })
                }
              >
            ...
            {form.schedule_type === 'interval' && (
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) =>
                    ff({ interval_value: e.target.value === '' ? '' : Number(e.target.value) })
                  }
                  sx={{ flex: 1 }}
                />
            ...
            {form.schedule_type === 'fixed_date' && (
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) =>
                    ff({ fixed_day: e.target.value === '' ? '' : Number(e.target.value) })
                  }
                  sx={{ flex: 1 }}
                />
                <TextField
                  ...
                  onChange={(e) =>
                    ff({ fixed_month: e.target.value === '' ? '' : Number(e.target.value) })
                  }
                  sx={{ flex: 1 }}
                />
              </Stack>
            ...
            <Box>
              <FormControlLabel
                control={
                  <Switch
                    size="small"
                    checked={form.next_due_date_auto}
                    onChange={(e) => {
            ...
            <TextField
              size="small"
              label="Giorni allerta"
              type="number"
              inputProps={{ min: 0 }}
              value={form.alert_days_before}
              fullWidth
              onChange={(e) =>
                ff({ alert_days_before: e.target.value === '' ? '' : Number(e.target.value) })
              }
            />
...
function NotificationsTab() {
  const toast = useToast()
  const grid = useServerGrid({
    defaultOrdering: '-sent_at',
    allowedOrderingFields: ['sent_at', 'due_date', 'updated_at'],
  })
  ...
  const columns: GridColDef<NotifRow>[] = React.useMemo(
    () => [
      { field: 'due_date', headerName: 'Scadenza', width: 130 },
      {
        ...
        width: 100,
        renderCell: (p) => (
          <Chip size="small" label={p.value} color={p.value === 'sent' ? 'success' : 'error'} />
        ),
      },
      { field: 'customer_code', headerName: 'Cliente', width: 90 },
      ...
      { field: 'plan_title', headerName: 'Piano', flex: 1, minWidth: 220 },
      {
        ...
        width: 180,
        valueGetter: (_v, row) =>
          row.sent_at ? new Date(row.sent_at).toLocaleString('it-IT') : '—',
      },
    ],
    [],
  )
    ...
    <>
      <EntityListCard
        toolbar={{
          ...
          onQChange: grid.setQ,
          onReset: () => {
            grid.reset(['n_customer', 'n_status'])
            setCustomerId('')
            setStatusF('')
          },
        }}
      ...
      >
        <Stack sx={{ p: 2 }} spacing={1.5}>
          <DetailDrawerHeader
            ...
            subtitle={
              detail?.customer_code
                ? `${detail.customer_code} — ${detail.inventory_hostname ?? ''}`
                : ''
            }
            onClose={() => grid.setOpenId(null)}
          />
            ...
            <>
              <Stack direction="row" spacing={0.75} flexWrap="wrap">
                <Chip
                  size="small"
                  label={detail.status}
                  color={detail.status === 'sent' ? 'success' : 'error'}
                />
              ...
              <FieldRow
                label="Data invio"
                value={detail.sent_at ? new Date(detail.sent_at).toLocaleString('it-IT') : ''}
              />
              {detail.error_message && (
                <>
                  <Divider />
                  <Typography variant="subtitle2" sx={{ opacity: 0.6, color: 'error.main' }}>
                    Errore
                  </Typography>
                  <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap' }}>
                    {detail.error_message}
...
function TodoTab() {
  ...
  const { rows: allTechs } = useDrfList<{ id: number; full_name: string }>('/techs/', {
    ordering: 'last_name',
    page_size: 500,
    is_active: 'true',
  })
  ...
  React.useEffect(() => {
    ...
    api
      .get<{ results: SiteItem[] }>('/sites/', {
        params: { customer: customerF, ordering: 'name', page_size: 500 },
      })
      .then((r) => setFilterSites(r.data.results ?? []))
      .catch(() => {})
  }, [customerF])
  ...
  const load = React.useCallback(() => {
    ...
    api
      .get<TodoRow[]>('/maintenance-plans/todo/', { params })
  ...
  React.useEffect(() => {
    if (!evForm.plan || !menuRow) {
      setPlanInvs([])
      return
    }
    api
      .get<{ results: InventoryItem[] }>('/inventories/', {
        params: { customer: menuRow.customer_id, ordering: 'hostname', page_size: 500 },
      })
      .then((r) => setPlanInvs(r.data.results ?? []))
      .catch(() => {})
  }, [evForm.plan, menuRow])

  const saveEvent = async () => {
    if (!evForm.plan) {
      toast.warning('Piano richiesto.')
      return
    }
    if (!evForm.inventory) {
      toast.warning('Inventario richiesto.')
      return
    }
    if (!evForm.performed_at) {
      toast.warning('Data richiesta.')
      return
    }
    if (!evForm.result) {
      toast.warning('Risultato richiesto.')
      return
    }
    if (!evForm.tech) {
      toast.warning('Tecnico richiesto.')
      return
    }
    setEvSaving(true)
    try {
      ...
      await api.post('/maintenance-events/', fd, {
              ...
              >
                <FormControl size="small" fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      setCustomerF(e.target.value === '' ? '' : Number(e.target.value))
                      setSiteF('')
                    }}
                  >
                    <MenuItem value="">Tutti</MenuItem>
            ...
            {(() => {
              ...
              const todoCols: GridColDef[] = [
                ...
                },
                {
                  ...
                  headerName: 'Sito',
                  width: 180,
                  valueGetter: (_v: any, row: any) => row.site_name ?? '—',
                },
                {
                  field: 'type_label',
                  headerName: 'Tipo',
                  width: 160,
                  valueGetter: (_v: any, row: any) => row.type_label ?? '—',
                },
                {
                  field: 'knumber',
                  headerName: 'K-number',
                ...
                },
                {
                  ...
                  width: 175,
                  renderCell: (p: GridRenderCellParams) => {
                    const overdue = (p.value as string) < today
                    return (
                      <Chip
                        size="small"
                ...
                { field: 'plan_title', headerName: 'Piano', width: 200 },
                {
                  field: '_act',
                  headerName: '',
                  width: 56,
                  ...
                  renderCell: (p: GridRenderCellParams) => (
                    <Tooltip title="Azioni">
                      <IconButton
                        aria-label="Azioni"
                        size="small"
                        onClick={(e) => openMenu(e, p.row as TodoRow)}
                      >
              ...
              const gridRows = rows.map((r) => ({ ...r, id: `${r.plan_id}-${r.inventory_id}` }))
              return (
                <DataGrid
                  rows={gridRows}
                  columns={todoCols}
                  ...
                  getRowClassName={(p) =>
                    (p.row as TodoRow).next_due_date < today ? 'todo-row-overdue' : ''
                  }
                  sx={{
                    ...GRID_SX,
                    '& .todo-row-overdue': { bgcolor: 'rgba(211,47,47,0.04)' },
                    '& .todo-row-overdue:hover': { bgcolor: 'rgba(211,47,47,0.08) !important' },
                  }}
                />
              )
            })()}
          ...
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            ...
            <FormControl size="small" fullWidth required>
              ...
              <Select
                ...
                onChange={(e) =>
                  evf({ inventory: e.target.value === '' ? '' : Number(e.target.value) })
                }
              >
                <MenuItem value="">—</MenuItem>
                {planInvs.map((i) => (
...
function EventsTab() {
  ...
  // `filterPlans` caricati per cliente selezionato nel filtro (evita caricamento globale di 500 piani).
  // Se nessun cliente è selezionato, la lista rimane vuota e il FilterChip "Piano" mostra solo "Tutti".
  ...
  const { rows: allTechs } = useDrfList<{ id: number; full_name: string; is_active: boolean }>(
    '/techs/',
    { ordering: 'last_name', page_size: 500, is_active: 'true' },
  )

  // Carica piani filtrati per cliente nel filtro lista
  React.useEffect(() => {
    if (!customerId) {
      setFilterPlans([])
      setPlanF('')
    ...
    api
      .get<{ results: { id: number; title: string }[] }>('/maintenance-plans/', {
        params: { customer: customerId, ordering: 'title', page_size: 500, is_active: 'true' },
      })
      .then((r) => setFilterPlans(r.data.results ?? []))
      .catch(() => {})
  }, [customerId])
  ...
  const [formPlans, setFormPlans] = React.useState<{ id: number; title: string }[]>([])
  const pdfInputRef = React.useRef<HTMLInputElement>(null)
  ...
  React.useEffect(() => {
    ...
    setDlLd(true)
    api
      .get<EventRow>(`/maintenance-events/${openId}/`)
  ...
  React.useEffect(() => {
    ...
    api
      ...
      .then((r) => setFormSites(r.data.results ?? []))
      .catch(() => {})
  }, [formCustomerId])

  // Carica piani filtrati per cliente nel form (evita filtro client-side su lista globale)
  React.useEffect(() => {
    if (!formCustomerId) {
      setFormPlans([])
      return
    ...
    api
      .get<{ results: { id: number; title: string }[] }>('/maintenance-plans/', {
        params: { customer: formCustomerId, ordering: 'title', page_size: 500, is_active: 'true' },
      })
      .then((r) => setFormPlans(r.data.results ?? []))
      .catch(() => {})
  }, [formCustomerId])
  ...
  React.useEffect(() => {
    ...
    const params: Record<string, any> = {
      customer: formCustomerId,
      ordering: 'hostname',
      page_size: 500,
    }
    if (formSiteId) params.site = formSiteId
    api
      .get<{ results: InventoryItem[] }>('/inventories/', { params })
  ...
  const openEdit = () => {
    ...
    api
      .get<any>(`/maintenance-plans/${detail.plan}/`)
      .then((r) => {
  ...
  const save = async () => {
    if (!form.plan) {
      toast.warning('Seleziona un piano.')
      return
    }
    if (!form.inventory) {
      toast.warning('Seleziona un inventario.')
      return
    }
    if (!form.performed_at) {
      toast.warning('Inserisci la data di esecuzione.')
      return
    }
    if (!form.result) {
      toast.warning('Seleziona il risultato.')
      return
    }
    if (!form.tech) {
      toast.warning('Seleziona un tecnico.')
      return
    }
  ...
  const doRestore = async () => {
    ...
    try {
      ...
      toast.success('Rapportino ripristinato ✅')
      reload()
      const r = await api.get<EventRow>(`/maintenance-events/${detail.id}/`)
      setDetail(r.data)
    } catch (e) {
      toast.error(apiErrorToMessage(e))
    } finally {
    ...
    <>
      <EntityListCard
        toolbar={{
          ...
          onViewModeChange: (v) => grid.setViewMode(v, { keepOpen: true }),
          onReset: () => {
            grid.reset(['ev_customer', 'ev_result', 'ev_plan'])
            setCustomerId('')
            setResultF('')
            setPlanF('')
          },
      ...
      <Dialog open={dlgOpen} onClose={() => setDlgOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>
          {dlgMode === 'create' ? 'Nuovo rapportino' : 'Modifica rapportino'}
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            ...
            {/* Piano filtrato per cliente (caricato server-side) */}
            <FormControl size="small" fullWidth required disabled={!formCustomerId}>
              ...
              <Select
                ...
                onChange={(e) =>
                  ff({ plan: e.target.value === '' ? '' : Number(e.target.value), inventory: '' })
                }
              >
                <MenuItem value="">—</MenuItem>
                {formPlans.map((p) => (
            ...
            <FormControl size="small" fullWidth required disabled={!formCustomerId}>
              ...
              <Select
                ...
                onChange={(e) =>
                  ff({ inventory: e.target.value === '' ? '' : Number(e.target.value) })
                }
              >
                <MenuItem value="">—</MenuItem>
...
function TechsTab() {
  ...
  const grid = useServerGrid({
    ...
    allowedOrderingFields: ['last_name', 'first_name', 'updated_at'],
  })
  ...
  const { rows, rowCount, loading, reload } = useDrfList<TechRow>('/techs/', listParams, (e) =>
    toast.error(apiErrorToMessage(e)),
  )
  ...
  const save = async () => {
    if (!form.first_name.trim() || !form.last_name.trim() || !form.email.trim()) {
      toast.warning('Nome, cognome ed email sono obbligatori.')
      return
    }
    const payload = {
    ...
    try {
      let id: number
      if (dlgMode === 'create') {
        const r = await api.post<TechRow>('/techs/', payload)
        id = r.data.id
        toast.success('Tecnico creato ✅')
      } else {
        const r = await api.patch<TechRow>(`/techs/${dlgId}/`, payload)
        id = r.data.id
  ...
  const doDelete = async () => {
    ...
    setDelBusy(true)
    try {
      await api.delete(`/techs/${detail.id}/`)
      toast.success('Tecnico eliminato ✅')
      grid.setViewMode('all', { keepOpen: true })
      reload()
    } catch (e) {
      toast.error(apiErrorToMessage(e))
  ...
  const doRestore = async () => {
    ...
    setResBusy(true)
    try {
      await api.post(`/techs/${detail.id}/restore/`)
      toast.success('Tecnico ripristinato ✅')
      reload()
      const r = await api.get<TechRow>(`/techs/${detail.id}/`)
      setDetail(r.data)
    } catch (e) {
      toast.error(apiErrorToMessage(e))
    } finally {
...
export default function Maintenance() {
  ...
  const tab: TabKey = (
    ['todo', 'plans', 'events', 'notifications', 'techs'].includes(tabParam) ? tabParam : 'todo'
  ) as TabKey
