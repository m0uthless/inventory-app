...
const NAV_ITEMS: { id: NavSection; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'permessi', label: 'Permessi & Gruppi', icon: <ShieldOutlinedIcon fontSize="small" /> },
  { id: 'attivita', label: 'Attività recente', icon: <HistoryIcon fontSize="small" /> },
]
  ...
  return (
    <Box>
      ...
      >
        ...
        <Box sx={{ minWidth: 0 }}>
          <Typography sx={{ fontSize: '0.8rem', fontWeight: 600, mb: 0.25 }}>
            Foto profilo
          </Typography>
          <Typography sx={{ fontSize: '0.7rem', color: 'text.secondary', mb: 1 }}>
            JPG, PNG o WEBP · max 2 MB · 1024×1024 px
          </Typography>
      ...
      <Stack spacing={1.25} sx={{ maxWidth: 320 }}>
        <TextField
          ...
          fullWidth
          size="small"
          helperText="Il nome utente non può essere modificato."
...
function SezioneSicurezza() {
  ...
  const validate = (): Record<string, string> => {
    ...
    if (newPwd && !newPwd2) errs.new_password2 = 'Conferma la nuova password.'
      ...
      errs.new_password2 = 'Le password non coincidono.'
    return errs
  }
      ...
      if (isRecord(e)) {
        ...
        if (isRecord(resp)) {
          ...
          if (isRecord(data)) {
            ...
            const oldP = pick('old_password')
            const newP = pick('new_password')
            const newP2 = pick('new_password2')
            if (oldP) mapped.old_password = oldP
            if (newP) mapped.new_password = newP
            if (newP2) mapped.new_password2 = newP2
          }
        }
      }
    ...
    <Box>
      ...
      >
        <Typography sx={{ fontSize: '0.8rem', color: 'text.secondary', lineHeight: 1.7 }}>
          Scegli una password di almeno <strong>8 caratteri</strong>. Dopo il cambio non verrai
          disconnesso dalla sessione corrente.
        </Typography>
      </Box>
      ...
      <Stack spacing={1.25} sx={{ maxWidth: 320 }}>
        <PasswordField
          ...
          onChange={(v) => {
            setOldPwd(v)
            setFieldErrors((e) => ({ ...e, old_password: '' }))
            setSuccessMsg(null)
          }}
        ...
        <PasswordField
          ...
          onChange={(v) => {
            setNewPwd(v)
            setFieldErrors((e) => ({ ...e, new_password: '' }))
            setSuccessMsg(null)
          }}
        ...
        <PasswordField
          ...
          onChange={(v) => {
            setNewPwd2(v)
            setFieldErrors((e) => ({ ...e, new_password2: '' }))
            setSuccessMsg(null)
          }}
...
function SezioneAttivita({ me }: { me: NonNullable<ReturnType<typeof useAuth>['me']> }) {
  ...
  const { rows, rowCount, loading } = useDrfList<AuditEventRow>(
    '/audit-events/',
    params,
    ...
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Button
          ...
          onClick={() => navigate(`/audit?actor=${me.id}`)}
          sx={{ fontSize: '0.72rem' }}
        >
          Vedi tutto
        </Button>
      ...
      {loading ? (
        ...
        >
          {rows.slice(0, 10).map((ev, idx) => {
            ...
            const obj = ev.object_repr || ev.subject || ev.object_id || '—'
            ...
            return (
              ...
              >
                <Box sx={{ pt: 0.25, pr: 1.5, flexShrink: 0 }}>
...
export default function Profile() {
  ...
  return <ProfileContent me={me} refreshMe={refreshMe} section={section} setSection={setSection} />
}

// ─── Dialog wrapper (aperto dalla navbar) ────────────────────────────────────
export function ProfileDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <Typography sx={{ fontWeight: 700, fontSize: '0.95rem' }}>Profilo utente</Typography>
...
// ─── Contenuto condiviso (pagina + dialog) ───────────────────────────────────
function ProfileContent({
    ...
    <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
      ...
      <Box
        sx={{
          ...
          flexShrink: 0,
          bgcolor: 'background.paper',
          borderRadius: isDialog ? 0 : 2.5,
          border: isDialog ? 'none' : '1px solid',
          borderColor: 'divider',
          borderRight: isDialog ? '1px solid' : undefined,
          borderRightColor: isDialog ? 'divider' : undefined,
          overflow: 'hidden',
          boxShadow: isDialog ? 'none' : '0 1px 6px rgba(15,23,42,0.06)',
        }}
      >
        {/* Sidebar top */}
        <Box
          sx={{
            background: `linear-gradient(155deg, ${TEAL_DARK} 0%, ${TEAL} 100%)`,
            px: 2,
            py: 2.5,
            display: 'flex',
        ...
        >
          ...
          <Box sx={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
            <Typography
              sx={{ fontSize: '0.825rem', fontWeight: 700, color: '#fff', lineHeight: 1.3 }}
            >
              {fullName}
            </Typography>
            <Typography sx={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.55)', mt: 0.25 }}>
              @{me.username}
            </Typography>
          </Box>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  '&:hover': { bgcolor: active ? 'rgba(15,118,110,0.08)' : 'rgba(0,0,0,0.04)' },
                }}
              >
                <Box
                  sx={{
        ...
        <Box sx={{ px: 2, py: 1.5 }}>
          <Typography sx={{ fontSize: '0.68rem', color: 'text.disabled', lineHeight: 1.8 }}>
            {me.permissions.length} permessi · {me.groups.length}{' '}
            {me.groups.length === 1 ? 'gruppo' : 'gruppi'}
          </Typography>
        </Box>
      </Box>
      ...
      <Box
        sx={{
          ...
          p: { xs: 2, sm: 3 },
          boxShadow: isDialog ? 'none' : '0 1px 6px rgba(15,23,42,0.06)',
          overflowY: isDialog ? 'auto' : undefined,
          maxHeight: isDialog ? 'calc(88vh - 56px)' : undefined,
        }}
      >
        {section === 'profilo' && <SezioneProfico me={me} onSaved={refreshMe} />}
  ...
  if (isDialog) {
    return (
      <Box sx={{ display: 'flex', overflowY: 'auto', maxHeight: 'calc(88vh - 56px)' }}>{inner}</Box>
    )
  }
  ...
  return (
    <Box sx={{ maxWidth: 980, mx: 'auto' }}>
      ...
      <Typography variant="body2" sx={{ opacity: 0.6, mb: 3 }}>
        Gestisci il tuo account, la sicurezza e visualizza la tua attività.
      </Typography>
      {inner}
    </Box>
  )
