...
function getErrorMessage(e: unknown): string {
  ...
  if (isObjectLike(e) && typeof e.message === 'string') return e.message
  return 'Errore meteo'
}

function isAbortError(e: unknown): boolean {
  if (e instanceof DOMException && e.name === 'AbortError') return true
  if (isObjectLike(e) && e.name === 'AbortError') return true
  return false
}
...
export default function WeatherWidget({ city }: { city: string }) {
  ...
  switch (state.status) {
    case 'idle':
    case 'loading':
      return <Chip size="medium" icon={<CloudIcon />} label="Meteo…" variant="filled" sx={chipSx} />
    ...
    case 'ready': {
      ...
      return (
        <Tooltip
          title={`${desc} • vento ${Math.round(windKmh)} km/h • agg. ${updatedAtIso} (click refresh)`}
        >
          <Chip
            size="medium"
