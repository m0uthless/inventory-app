...
export type AuditChange = { from?: unknown; to?: unknown; before?: unknown; after?: unknown }
...
function normFrom(change: AuditChange | undefined): unknown {
  return change && ('from' in change || 'to' in change) ? change.from : change?.before
}

function normTo(change: AuditChange | undefined): unknown {
  return change && ('from' in change || 'to' in change) ? change.to : change?.after
}
...
export default function AuditDiffTable(props: Props) {
  ...
  const rows = React.useMemo(() => {
    ...
    return Object.entries(changes || {})
      .filter(([, ch]) => !isSame(normFrom(ch), normTo(ch)))
        ...
        <TableBody>
          {rows.map(([field, ch]) => (
            <TableRow key={field}>
              ...
              <TableCell>{fmtValue(normFrom(ch))}</TableCell>
              <TableCell>{fmtValue(normTo(ch))}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
