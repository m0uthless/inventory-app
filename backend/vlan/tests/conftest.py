...
AuslBoScopedMixin e le permission class IsAuslBoUserOrInternal/AuslBoModelPermissions.
"""
import uuid
...
# AUSLBO/Device/VLAN. Usati da make_internal_user/make_auslbo_user quando
# can_edit=True, per riflettere la matrice reale applicata da
# AuslBoModelPermissions (view/add/change/delete_<model>), non più il solo
# "device.change_device" che bastava con il vecchio IsAuslBoEditor.
...
def make_internal_user(*, can_edit: bool = True, extra_perms: list[str] | None = None) -> "User":
    ...
    Rispacs/DeviceWifi/DeviceRispacs (richiesta da AuslBoModelPermissions
...
def make_auslbo_user(customer: Customer, *, can_edit: bool = False, extra_perms: list[str] | None = None) -> "User":
    """Utente portale AUSL BO "puro" (nessun accesso interno Archie), con
    AuslBoUserProfile associato al customer indicato. Vede/scrive SEMPRE
    solo i dati del proprio customer, indipendentemente da qualunque header
    (fix 2.1, audit 2026-07: lo scope non dipende più da X-Auslbo-Portal)."""
...
def make_dual_user(customer: Customer, *, can_edit: bool = False) -> "User":
    """Utente "duale": ha SIA un profilo AUSL BO (associato a `customer`) SIA
    ...
    user = make_auslbo_user(customer, can_edit=can_edit)
    user.user_permissions.add(Permission.objects.get(codename="access_archie"))
    return user
