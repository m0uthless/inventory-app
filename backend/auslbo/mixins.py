...
from auslbo.permissions import _is_auslbo_user, _is_internal_user, _get_auslbo_customer_id
...
class AuslBoScopedMixin:
    ...
    - utente con accesso interno Archie (`core.access_archie` o superuser)
      → nessuno scope, vede tutto, anche se ha anche un profilo AUSL BO;
    - utente AUSL BO "puro" (profilo attivo, nessun accesso interno)
    ...
    def filter_queryset(self, queryset):
        ...
        # AUSL BO): non viene scopato. Questa decisione è presa SOLO in base
...
class AuslBoTenantWriteMixin:
    """Mixin da aggiungere ai ViewSet insieme a `AuslBoScopedMixin` per
    vincolare le SCRITTURE (create/update) al tenant dell'utente portale.

    `AuslBoScopedMixin` filtra solo le letture (list/retrieve): senza questo
    ...
    def _portal_customer_id(self):
        user = getattr(self.request, "user", None)
        if not _is_auslbo_user(user) or _is_internal_user(user):
            return None
        return _get_auslbo_customer_id(user)
