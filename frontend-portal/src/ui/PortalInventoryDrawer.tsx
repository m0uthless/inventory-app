...
// PortalInventoryDetail estende InventoryReadDetail senza aggiungere campi
// (alias esplicito per chiarezza nei consumer)
export type PortalInventoryDetail = InventoryReadDetail

export interface PortalInventoryDrawerProps {
  id: number | null
  onClose: () => void
}

export default function PortalInventoryDrawer({ id, onClose }: PortalInventoryDrawerProps) {
  const open = id !== null
  const [drawerTab, setDrawerTab] = React.useState(0)
  const [detail, setDetail] = React.useState<PortalInventoryDetail | null>(null)
  ...
  React.useEffect(() => {
    ...
    api.get<PortalInventoryDetail>(`/inventories/${id}/`)
