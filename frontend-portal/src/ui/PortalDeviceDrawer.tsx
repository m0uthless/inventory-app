...
export type PortalDeviceDetail = DeviceReadDetail & {
  id: number
...
export interface PortalDeviceDrawerProps {
  id: number | null
  onClose: () => void
}

export default function PortalDeviceDrawer({ id, onClose }: PortalDeviceDrawerProps) {
  const open = id !== null
  const [drawerTab, setDrawerTab] = React.useState(0)
  const [detail, setDetail] = React.useState<PortalDeviceDetail | null>(null)
  ...
  React.useEffect(() => {
    ...
    api
      .get<PortalDeviceDetail>(`/devices/${id}/`)
