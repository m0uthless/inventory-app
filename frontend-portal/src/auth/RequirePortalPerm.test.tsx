...
import { RequirePortalPerm } from './RequirePortalPerm'
import type { PortalMe } from './AuthProvider'

// ─── Mock useAuth ───────────────────────────────────────────────────────────
// Ogni test imposta il valore restituito tramite mockUseAuthReturn.

let mockUseAuthReturn: { me: PortalMe | null; loading: boolean } = { me: null, loading: true }

vi.mock('./AuthProvider', () => ({
  useAuth: () => mockUseAuthReturn,
}))

function makeMe(permissions: string[]): PortalMe {
  return {
    ...
    customers: [{ id: 1, name: 'Cliente Alpha', display_name: 'Cliente Alpha', code: 'C-001' }],
    portal: { is_active: true, can_edit_devices: false, permissions },
  }
}
...
describe('RequirePortalPerm — route guard portale AUSL BO (fix P0 6.6)', () => {
