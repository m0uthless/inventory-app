...
import MobileBottomNavPortal from './MobileBottomNavPortal'
...
type NavItem = {
   ...
   * lato server da PortalModelPermissions). Se assente, visibile a chiunque
   * sia autenticato. */
  requiredPerm?: string
}
...
export function AppLayout() {
  ...
  const grantedPerms = me?.portal.permissions ?? []
  ...
  const [desktopOpen, setDesktopOpen] = React.useState(() => {
    const v = localStorage.getItem('portal_sidebar_open')
    return v ? v === '1' : true
  })
  React.useEffect(() => {
    localStorage.setItem('portal_sidebar_open', desktopOpen ? '1' : '0')
  }, [desktopOpen])
    ...
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
      ...
      >
        <Toolbar sx={{ pl: 2, pr: 1, gap: 1 }}>
          ...
          {/* RIGHT: selettore cliente (solo se >1 assegnato) + avatar */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 'auto' }}>
            {me && me.customers.length > 1 && (
              <>
                <Tooltip title="Cambia cliente">
                  <Box
                    component="button"
                    onClick={(e) => setCustomerAnchorEl(e.currentTarget)}
                    disabled={switchingCustomer}
                    sx={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: 0.75,
                      ...
                      color: SIDEBAR.textStrong,
                      cursor: switchingCustomer ? 'default' : 'pointer',
                      opacity: switchingCustomer ? 0.6 : 1,
                      font: 'inherit',
                      '&:hover': { background: 'rgba(255,255,255,0.12)' },
                    }}
                    aria-label="Cambia cliente attivo"
                  >
                ...
                >
                  {me.customers.map((c) => (
                    <MenuItem
                      key={c.id}
                      selected={c.id === me.customer.id}
                      onClick={() => {
                        setCustomerAnchorEl(null)
                        if (c.id !== me.customer.id) void switchCustomer(c.id)
                      }}
