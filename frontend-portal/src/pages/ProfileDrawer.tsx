...
import { useAuth, type PortalMe } from '../auth/AuthProvider'
...
function SezioneProfilo({ me, onSaved }: { me: PortalMe; onSaved: () => Promise<void> }) {
...
function SezioneAccesso({ me }: { me: PortalMe }) {
  const [showAll, setShowAll] = React.useState(false)
  const perms = me.portal.permissions ?? []
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          <Chip
            label={me.portal.is_active ? '● Attivo' : '● Inattivo'}
            size="small"
            sx={{
              bgcolor: me.portal.is_active ? 'rgba(16,185,129,0.10)' : 'rgba(239,68,68,0.10)',
              color:   me.portal.is_active ? theme.palette.success.dark : theme.palette.error.dark,
              border:  `1px solid ${me.portal.is_active ? 'rgba(16,185,129,0.28)' : 'rgba(239,68,68,0.28)'}`,
              fontWeight: 700, fontSize: '0.72rem',
            }}
          />
          {me.portal.can_edit_devices && (
    ...
    >
      ...
      >
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5, position: 'relative', zIndex: 2 }}>
          <Chip
            size="small"
            label={me.portal.can_edit_devices ? '● Editor' : '● Utente'}
