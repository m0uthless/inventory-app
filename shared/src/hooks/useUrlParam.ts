...
export type UrlPatch = Record<string, unknown>
...
export function useUrlArrayParam<T extends string = string>(
  ...
  const defaultValue = (options?.defaultValue ?? EMPTY_ARRAY) as readonly T[]

  // Pull out `allowed` so React Compiler can correctly track memo deps.
  // (Accessing `options?.allowed` inside the memo callback makes the inferred dep `options`.)
  ...
  const setValue = React.useCallback(
    (nextValue: readonly T[], opts?: UrlSetOptions) => {
      setSp(
        (prev) => {
          ...
          if (canonicalize) items = [...items].sort()
    ...
    [setSp, key, separator, allowedSet, canonicalize],
  )
...
export function useUrlNumberArrayParam(
  ...
  const [sp, setSp] = useSearchParams()
  const separator = options?.separator ?? ','
  const defaultValue = (options?.defaultValue ?? EMPTY_ARRAY) as readonly number[]
  ...
  const setValue = React.useCallback(
    (nextValue: readonly number[], opts?: UrlSetOptions) => {
      setSp(
        (prev) => {
          ...
          if (canonicalize) items = [...items].sort((a, b) => a - b)
    ...
    [setSp, key, separator, canonicalize],
  )
